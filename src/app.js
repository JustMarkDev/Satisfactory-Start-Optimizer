...
// Game Map Constants (Centimeter Units)
const MIN_X = -320000.0;
const MAX_X = 420000.0;
const MIN_Y = -370000.0;
const MAX_Y = 370000.0;

// World Border Polygon vertices (Satisfactory Map coordinates)
const WORLD_BORDER_POLYGON = [
  [-232400, -370000],
  [450000, -370000],
  [450000, -33000],
  [192000, 335000],
...
// Pixel coordinate inside a 1280x1280 element -> Game Coordinate
function pixelToGame(px, py) {
  const gx = (px - 571.321) / 0.0013653321;
  const gy = (py - 640.0) / 0.0013653333;
  return { x: gx, y: gy };
}
            ...
            subcat.options.forEach(item => {
              ...
              if (item.markers) {
                const addMarker = (marker) => {
                  ...
                  switch (rawType) {
                    case "Desc_Stone_C": mType = "limestone"; break;
                    case "Desc_OreIron_C": mType = "iron"; break;
                    case "Desc_OreCopper_C": mType = "copper"; break;
                    case "Desc_OreGold_C": mType = "caterium"; break;
                    case "Desc_Coal_C": mType = "coal"; break;
                    case "Desc_LiquidOil_C": mType = "oil"; break;
                    case "Desc_Sulfur_C": mType = "sulfur"; break;
                    case "Desc_OreBauxite_C": mType = "bauxite"; break;
                    case "Desc_RawQuartz_C": mType = "quartz"; break;
                    case "Desc_OreUranium_C": mType = "uranium"; break;
                    case "Desc_SAM_C": mType = "sam"; break;
                    case "Desc_NitrogenGas_C": mType = "nitrogenwell"; break;
                    case "Desc_Water_C": mType = "waterwell"; break;
                    default:
  ...
  try {
    ...
    const reqBody = {
      ...
      weights: Object.fromEntries(
        Object.entries(config.weights).filter(([_, v]) => v !== 0)
      )
    };
...
function renderMapOverlay() {
  ...
  const activeKeys = Object.keys(state.config.weights).filter(k => Math.abs(state.config.weights[k]) > 0);
  ...
  if (state.visibleLayers.nodes && state.rawNodes.length > 0) {
    state.rawNodes.forEach(node => {
      ...
      const purityStr = node.purityMultiplier > 1.5 ? "Pure" : (node.purityMultiplier < 0.8 ? "Impure" : "Normal");
  ...
  // Draw World Border Polygon
  const borderPoints = WORLD_BORDER_POLYGON.map(pt => {
  ...
  borderPoly.setAttribute("stroke", "rgba(255, 50, 50, 0.45)");
  ...
  DEFAULT_SPAWNS.forEach(spawn => {
    ...
    boundaryCircle.setAttribute("fill", ignoreSpawns ? "rgba(255, 152, 0, 0.01)" : "rgba(255, 152, 0, 0.06)");
    boundaryCircle.setAttribute("stroke", ignoreSpawns ? "rgba(255, 152, 0, 0.15)" : "rgba(255, 152, 0, 0.45)");
  ...
  state.results.forEach((res, idx) => {
    ...
    const tooltipText = `
      <div class="title" style="color: ${color}">Site #${idx + 1} (Optimal)</div>
      <div style="font-weight: 700; margin-bottom: 4px;">Score: <span style="color: var(--color-secondary);">${res.score.toFixed(4)}</span></div>
      <div style="font-size: 0.75rem; margin-bottom: 4px;">Closest Biome: ${res.closest_spawn.name} (${Math.round(res.spawn_distance).toLocaleString()}m)</div>
      <div class="coordinates">X: ${Math.round(res.x).toLocaleString()}, Y: ${Math.round(res.y).toLocaleString()}, Z: ${Math.round(res.z / 100.0)}m</div>
    `;
...
function renderResultsPanel() {
  ...
  if (state.results.length === 0) {
    els.resultsList.innerHTML = '<div style="padding: 24px; color: var(--color-text-muted); text-align: center; font-size: 0.85rem; font-family: var(--font-display); letter-spacing: 1px;">CLICK "COMPUTE STARTING AREAS" TO RUN SIMULATION</div>';
    return;
  }
...
function renderWeightSliders() {
  ...
  RESOURCES.forEach(res => {
    const val = state.config.weights[res.id] !== undefined ? state.config.weights[res.id] : 0.00;
    const isExcluded = (val === 0.00);
